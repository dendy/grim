...
macro( grim_create_translation QM_FILE TS_TARGETS TS_FILE )
	...
	if ( _source_files )
		...
		set( _ts_pro "${CMAKE_CURRENT_BINARY_DIR}/${_ts_filename_we}.pro" )

		file( WRITE "${_ts_pro}"
			"#\n"
			"# This file is autogenerated by frim_create_translation() macro\n"
		...
		foreach ( _source_file ${_source_files} )
			file( APPEND "${_ts_pro}" "  ${_source_file}\\\n" )
	...
	set( _ts_target "${CMAKE_CURRENT_BINARY_DIR}/${_ts_filename_we}_non_existed" )
	...
	add_custom_command(
		OUTPUT
			${_ts_target}
		COMMAND
			${QT_LUPDATE_EXECUTABLE} ${_lupdate_options} "${_ts_pro}" -ts "${_ts_file_absolute}"
		DEPENDS
			${_source_files}
			${_ts_pro}
		)
	...
	if ( NOT EXISTS "${_ts_file_absolute}" )
		execute_process( COMMAND ${QT_LUPDATE_EXECUTABLE} ${_lupdate_options} "${_ts_pro}" -ts "${_ts_file_absolute}" )
...
macro( grim_add_translation QM_FILE TS_FILE )
	get_filename_component( _ts_file_absolute "${TS_FILE}" ABSOLUTE )
...
macro( grim_add_precompiled_headers TARGET PRECOMPILED_HEADER )
	...
	if ( MSVC )
		...
		set( _target_pdb "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${_target_filename}.pdb" )
		add_custom_command(
...
macro( grim_get_target_libraries INTERFACE_LIBRARY RUNTIME_LIBRARY TARGET )
	...
	if ( "${_target_type}" STREQUAL "STATIC_LIBRARY" )
		...
		if ( NOT _target_interface_prefix )
			set( _target__interface_prefix ${CMAKE_STATIC_LIBRARY_PREFIX} )
